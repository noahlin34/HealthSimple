...
export default function AppointmentsItem({label, subheading, date, onPress}: Props) {
    ...
    return ( 
        <Pressable onPress={onPress} onPressIn={() => setIsPressed(true)} onPressOut={() => setIsPressed(false)}>
            <View style={isPressed ? styles.containerPressed : styles.container}>
                ...
                <View style={styles.verticalContainer}>
                    <Text style={Styles.label}>{theDate.toLocaleDateString('en-us', {month: 'long', day: 'numeric'})}</Text>
                    <Text style={Styles.labelSmall}>{theDate.toLocaleTimeString("en-US", {hour: 'numeric', minute: '2-digit'})}</Text>
                </View>
